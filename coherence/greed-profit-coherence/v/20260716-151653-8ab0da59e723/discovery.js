(function (root) {
  ...
  function mount(container, context) {
    ...
    function render(focusBucket) {
      ...
      tbody.innerHTML = visible.length ? visible.map((item) => { const gap = Number.isFinite(item.greed_score) && Number.isFinite(item.support_score) ? item.greed_score - item.support_score : null; return `<tr tabindex="0" data-code="${esc(item.code)}" aria-label="${esc(`${item.name} ${item.code} 기업 분석 열기`)}"><td><strong>${esc(item.name)}</strong><small>${esc(item.code)} · ${esc(item.market)} · ${esc(item.sector_name || "업종 없음")}</small><small>기준 ${esc(item.market_cap_as_of || context.universe.diagnostic_as_of)}</small></td><td>${esc(STATE_LABEL[item.state] || "자료 부족")}<small>신뢰도 ${esc(item.confidence)} · ${esc(item.coverage_mode)}</small></td><td>${esc(cap(item.market_cap_krw))}</td><td>${compositeScore(item, "greed")}</td><td>${compositeScore(item, "support")}</td><td>${score(gap)}</td></tr>`; }).join("") : '<tr><td colspan="6" class="empty">조건에 맞는 종목이 없습니다.</td></tr>';
