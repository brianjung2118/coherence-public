(function (root) {
  ...
  const SERIES = {
    attention: {label: "시장 관심", className: "attention", dash: ""},
  ...
  function selectedHistory(history, range) {
    ...
    return {
      edm: allEdm.filter((row) => within(row.diagnostic_date)),
      earnings: allEarnings.filter((row) => within(row.period_date)),
    };
  }
  ...
  function historyTable(container, domain) {
    ...
    table.innerHTML = "<thead><tr><th>구분</th><th>기간</th><th>시장 관심</th><th>증권사 기대</th><th>실적 이행</th><th>기대 영업이익</th><th>실제 영업이익</th><th>출처 기준</th></tr></thead>";
    const body = el("tbody");
    domain.edm.forEach((row) => {
      const tr = el("tr"); tr.dataset.kind = "edm";
      ["EDM", row.diagnostic_date, fmtNumber(row.attention_score), fmtNumber(row.expectations_score), fmtNumber(row.delivery_score), "—", "—", `관심 ${row.attention_public_date || "없음"} · 기대 ${row.expectations_public_date || "없음"} · 이행 ${row.delivery_public_date || "없음"}`].forEach((value) => tr.append(el("td", null, value)));
      body.append(tr);
    });
    domain.earnings.forEach((row) => {
  ...
  function legend() {
    ...
    [
      ["attention", "시장 관심"], ["expectations", "증권사 기대"], ["delivery", "실적 이행"],
  ...
  function freshnessStatus(domain, company) {
    ...
    if (!latestEdm) edm.textContent = "EDM 이력 · 이용 불가";
      ...
      const partial = domain.edm.some((row) => [row.attention_score, row.expectations_score, row.delivery_score].some((value) => !finite(value)));
      edm.className = stale ? "stale" : "";
      edm.textContent = `EDM 이력 · 최신 ${latestEdm.diagnostic_date} · ${stale ? "최신 이력 지연" : "정상"}${partial ? " · 일부 자료 공백" : ""}`;
    }
    const earnings = el("p");
    ...
    if (!latestEarnings) earnings.textContent = "영업이익 이력 · 이용 불가";
      ...
      const partial = domain.earnings.some((row) => !finite(row.expected_operating_profit_krw) || !finite(row.actual_operating_profit_krw));
      earnings.className = stale ? "stale" : "";
      earnings.textContent = `영업이익 · 최근 분기 ${latestEarnings.fiscal_year}Q${latestEarnings.fiscal_quarter} · 공개 ${publicDate || "일자 없음"} · ${stale ? "최신 이력 지연" : "정상"}${partial ? " · 일부 자료 공백" : ""}`;
    }
  ...
  function renderCoordinatedHistory({container, company, range = "24m"}) {
    ...
    if (!domain.edm.length && !domain.earnings.length) {
    ...
    const allDates = [
      ...domain.edm.map((row) => row.diagnostic_date),
      ...domain.earnings.flatMap((row) => [row.period_date, row.actual_public_date]).filter(Boolean),
    ];
    ...
    const scores = scorePanel(domain.edm, range, allDates);
    const earnings = earningsPanel(domain.earnings, range, allDates);
    container.append(scores.svg, earnings.svg);
    const tip = tooltip(container);
    bindPanelPointer(scores.svg, container, domain.edm.length ? domain.edm : domain.earnings, tip, domain.edm.length ? edmTooltip : earningsTooltip);
    bindPanelPointer(earnings.svg, container, domain.earnings.length ? domain.earnings : domain.edm, tip, domain.earnings.length ? earningsTooltip : edmTooltip);
