  ...
  function mount(container, context) {
    ...
    container.innerHTML = `<section class="company-workspace" data-workspace="company">
      ...
      <section class="primary-chart-card" data-company-chart="edm">
        <div class="chart-card-heading"><div><p class="eyebrow">전체 이력</p><h2>EDM 흐름</h2><p>탐욕과 사업 근거를 먼저 보고, 필요할 때 여섯 세부 지표를 확인합니다.</p></div></div>
        <div class="chart-toolbar"><div class="range-control" role="group" aria-label="차트 보기"><button type="button" data-history-view="overview" aria-pressed="true">전체 흐름</button><button type="button" data-history-view="pillars" aria-pressed="false">세부 지표</button></div><div class="range-control" role="group" aria-label="표시 빈도"><button type="button" data-frequency="daily" aria-pressed="false">일간</button><button type="button" data-frequency="weekly" aria-pressed="true">주간</button><button type="button" data-frequency="monthly" aria-pressed="false">월간</button></div></div>
        <div class="pillar-chart-controls" data-pillar-chart-controls hidden>${[["retail_heat", "개인 관심 열기"], ["narrative", "서사 증폭"], ["attention", "시장 추격"], ["information", "정보 자극"], ["expectations", "증권사 기대"], ["delivery", "실적 이행"]].map(([value, label]) => `<button type="button" data-pillar-toggle="${value}" aria-pressed="true">${label}</button>`).join("")}</div>
    ...
    if (root.EDMCharts) {
      ...
      const controls = container.querySelector("[data-pillar-chart-controls]");
      const state = {view: "overview", frequency: "weekly", pillars: new Set(["retail_heat", "narrative", "attention", "information", "expectations", "delivery"])};
      const renderHistory = () => root.EDMCharts.renderCoordinatedHistory({container: chart, company: payload, view: state.view, frequency: state.frequency, selectedPillars: [...state.pillars]});
      container.querySelectorAll("[data-history-view]").forEach((button) => button.addEventListener("click", () => {
        state.view = button.dataset.historyView;
        container.querySelectorAll("[data-history-view]").forEach((item) => item.setAttribute("aria-pressed", String(item === button)));
        controls.hidden = state.view !== "pillars";
        renderHistory();
      }));
      container.querySelectorAll("[data-frequency]").forEach((button) => button.addEventListener("click", () => {
        state.frequency = button.dataset.frequency;
        container.querySelectorAll("[data-frequency]").forEach((item) => item.setAttribute("aria-pressed", String(item === button)));
        renderHistory();
      }));
      container.querySelectorAll("[data-pillar-toggle]").forEach((button) => button.addEventListener("click", () => {
        const key = button.dataset.pillarToggle;
        if (state.pillars.has(key) && state.pillars.size > 1) state.pillars.delete(key); else state.pillars.add(key);
        button.setAttribute("aria-pressed", String(state.pillars.has(key)));
        renderHistory();
      }));
      renderHistory();
    }
