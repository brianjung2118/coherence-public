(function (root) {
  ...
  const COMPONENT_HELP = {
    retail_post_count: "해당 날짜 네이버 종목토론실에 작성된 게시글 수입니다.",
    retail_views_per_post: "해당 날짜 네이버 종목토론실 조회 수를 게시글 수로 나눈 값입니다.",
    retail_engagement_per_post: "해당 날짜 네이버 종목토론실 댓글·좋아요·싫어요를 게시글 수로 나눈 값입니다.",
    retail_post_acceleration: "해당 날짜 토론 글 수가 직전 30일의 보통 수준보다 얼마나 늘거나 줄었는지 보여줍니다.",
    retail_heat_own_pct: "최근 최대 252일 중 해당 날짜 토론 글 수가 어느 정도로 높은지 나타낸 0~100점입니다.",
    retail_heat_market_pct: "같은 날짜 다른 종목과 비교한 토론 증가·조회·반응 수준의 0~100점입니다.",
    news_volume_acceleration: "해당 날짜 네이버 뉴스 기사 수가 직전 30일의 보통 수준보다 얼마나 늘거나 줄었는지 보여줍니다.",
    narrative_volume_pct: "기사 증가율을 같은 달 다른 종목과 비교한 0~100점입니다.",
    news_duplication_ratio: "이번 달 네이버 뉴스 중 내용이 서로 비슷하게 반복된 기사의 비중입니다.",
    narrative_duplication_pct: "반복 기사 비중을 같은 달 다른 종목과 비교한 0~100점입니다.",
    news_source_concentration: "이번 달 네이버 뉴스가 소수 언론사에 얼마나 몰려 있는지 보여줍니다.",
    narrative_source_concentration_pct: "언론사 쏠림을 같은 달 다른 종목과 비교한 0~100점입니다.",
    news_sentiment_extremity: "이번 달 네이버 뉴스 제목의 표현이 평범한 수준에서 얼마나 벗어났는지 보여줍니다.",
    narrative_sentiment_pct: "표현의 강도를 같은 달 다른 종목과 비교한 0~100점입니다.",
    attention_trading: "해당 거래일 거래량과 거래대금 회전이 다른 종목보다 얼마나 뜨거운지 나타낸 점수입니다.",
    attention_retail_credit: "해당 거래일 개인 순매수와 공개된 신용잔고가 다른 종목보다 얼마나 강한지 나타낸 점수입니다.",
    attention_behavior: "해당 거래일 최근 주가 상승과 고점 추격 움직임이 다른 종목보다 얼마나 강한지 나타낸 점수입니다.",
    news_article_count: "해당 날짜 네이버 뉴스에서 수집된 해당 기업 기사 수입니다.",
    news_unique_story_share: "한 달 기사 중 서로 다른 이야기로 분류된 기사의 비중입니다.",
    information_unique_story_pct: "고유 기사 비중을 같은 달 다른 종목과 비교한 0~100점입니다.",
    news_evidence_share: "한 달 기사 중 실적·수주·제품 등 구체적인 사업 사실을 담은 기사의 비중입니다.",
    information_evidence_pct: "구체적 사업 근거 비중을 같은 달 다른 종목과 비교한 0~100점입니다.",
    op_revision: "같은 증권사들의 최근 영업이익 전망이 이전 전망보다 얼마나 바뀌었는지 보여줍니다.",
    op_growth: "현재 증권사 영업이익 전망이 이전 사업연도보다 얼마나 늘거나 줄 것으로 보는지 보여줍니다.",
    revision_breadth: "비교 가능한 증권사 중 영업이익 전망을 올린 곳의 비율입니다.",
    op_dispersion: "증권사별 영업이익 전망이 서로 얼마나 다른지 보여줍니다. 높을수록 의견 차이가 큽니다.",
    brokers: "현재 영업이익 전망을 제공한 증권사 수입니다.",
    matched_brokers: "현재 전망과 이전 전망을 같은 증권사끼리 비교할 수 있었던 증권사 수입니다.",
    fiscal_horizon: "증권사 전망이 가리키는 사업연도와 기간입니다.",
    operating_profit_growth: "최근 네 분기 영업이익 합계가 그 이전 네 분기보다 얼마나 늘거나 줄었는지 보여줍니다.",
    operating_margin_change: "최근 네 분기의 매출 대비 영업이익률이 1년 전보다 얼마나 바뀌었는지 보여줍니다.",
    op_surprise: "발표된 영업이익이 발표 직전 증권사 전망보다 얼마나 높거나 낮았는지 보여줍니다.",
    delivery_metric_count: "실적 이행 점수 계산에 실제로 사용할 수 있었던 실적 지표 수입니다.",
  };
  ...
  function pillarDetail(company, key) {
    ...
    const rows = detail.components.map(([metricKey, label, kind]) => {
      ...
      const help = COMPONENT_HELP[metricKey] || `${label}이 무엇을 나타내는지 설명합니다.`;
      const tooltipId = `component-help-${key}-${metricKey}`;
      return `<div><dt><span class="component-name">${escapeHtml(label)}<button type="button" class="component-info" data-component-info aria-label="${escapeHtml(label)} 설명" aria-describedby="${escapeHtml(tooltipId)}">i</button><span id="${escapeHtml(tooltipId)}" class="component-tooltip" role="tooltip" aria-label="${escapeHtml(label)} 설명">${escapeHtml(help)}</span></span></dt><dd>${escapeHtml(formatDetailValue(item && item.value, kind))}</dd></div>`;
    }).join("");
  ...
  function signalSummary(company) {
    const attention = pillar(company, "attention");
    const expectations = pillar(company, "expectations");
    const delivery = pillar(company, "delivery");
    ...
    let diagnosis = {label: "판단 보류", body: "현재 자료로 탐욕과 사업 근거의 관계를 충분히 판단하기 어렵습니다.", tone: "neutral"};
    if (Number.isFinite(greed.value) && Number.isFinite(support.value)) {
      const gap = support.value - greed.value;
      const gapText = Math.abs(gap).toFixed(1);
      if (greed.value >= 60 && support.value >= 60) {
        diagnosis = gap >= 5
          ? {label: "근거 있는 열기", body: `사업 근거가 탐욕보다 ${gapText}점 앞섭니다. 시장의 열기를 설명할 근거가 더 강합니다.`, tone: "supported"}
          : gap <= -5
            ? {label: "근거를 확인할 열기", body: `탐욕이 사업 근거보다 ${gapText}점 앞섭니다. 기대 부담을 더 확인하세요.`, tone: "caution"}
            : {label: "근거 있는 열기", body: "탐욕과 사업 근거가 비슷한 수준으로 함께 움직입니다.", tone: "supported"};
      } else if (greed.value >= 60) diagnosis = {label: "기대 과열 주의", body: `탐욕이 사업 근거보다 ${gapText}점 앞섭니다. 아직 설명되지 않은 기대가 큽니다.`, tone: "caution"};
      else if (support.value >= 60) diagnosis = {label: "덜 주목받는 실적", body: `사업 근거가 탐욕보다 ${gapText}점 앞섭니다. 관심보다 사업 근거가 먼저 형성되고 있습니다.`, tone: "opportunity"};
      else diagnosis = {label: "관망 구간", body: "탐욕과 사업 근거가 모두 뚜렷하지 않습니다. 방향이 형성되는지 지켜보세요.", tone: "neutral"};
    }
    return {greed, support, diagnosis, gap: Number.isFinite(greed.value) && Number.isFinite(support.value) ? greed.value - support.value : null};
  }

  function gapHero(company) {
    const {greed, support, diagnosis, gap} = signalSummary(company);
    const gapText = Number.isFinite(gap) ? `${gap > 0 ? "+" : ""}${gap.toFixed(1)}` : "—";
    const position = Number.isFinite(gap) ? Math.max(0, Math.min(100, gap + 50)) : 50;
    return `<section class="edm-gap-hero ${escapeHtml(diagnosis.tone)}" data-edm-gap-hero data-edm-diagnosis>
      <div class="gap-hero-copy"><p class="eyebrow">Expectation–Delivery Monitor</p><h2>시장 탐욕이 사업 근거보다 앞서가고 있는가?</h2><p>시장의 열기가 사업의 현실보다 앞섰는지, 뒤처졌는지를 보여줍니다.</p></div>
      <div class="gap-hero-reading"><span>${escapeHtml(diagnosis.label)}</span><strong>${escapeHtml(gapText)}</strong><small>탐욕 − 사업 근거</small></div>
      <div class="gap-scale" style="--gap-position:${position}%"><div class="gap-scale-labels"><span>사업 근거 선행</span><span>균형</span><span>시장 탐욕 선행</span></div><div class="gap-scale-track"><i></i></div></div>
      <p class="gap-hero-story">${escapeHtml(diagnosis.body)}</p>
    </section>`;
  }
  ...
  function signalOverview(company) {
    ...
    const information = metric(attention, "information_impulse_score");
    const marketChase = attention && attention.score;
    const {greed, support} = signalSummary(company);
    const greedCards = [
    ...
    return `<details class="edm-breakdown" data-edm-breakdown><summary>왜 이렇게 나왔나요?</summary><section class="evidence-overview"><div class="breakdown-heading"><p>두 점수와 여섯 지표</p><small>각 지표를 누르면 뜻과 산출 근거를 볼 수 있습니다.</small></div><div class="signal-group"><h3><span>탐욕</span><strong>${escapeHtml(formatScore(greed.value))} · ${escapeHtml(greed.coverage)}</strong></h3><div class="signal-grid">${greedCards}</div></div><div class="signal-group support"><h3><span>사업 근거</span><strong>${escapeHtml(formatScore(support.value))} · ${escapeHtml(support.coverage)}</strong></h3><div class="signal-grid">${supportCards}</div></div><div class="pillar-detail-host" aria-live="polite"></div></section></details>`;
  }
  ...
  function sourceWarnings(company, diagnosticDate) {
    ...
    return `<details class="source-warning-disclosure" data-source-warnings><summary>자료 품질 ${warnings.length}건</summary><ul class="source-warnings">${warnings.map((warning) => { const detail = warningDetail(warning, company, diagnosticDate); const shownDate = detail.dateKind ? `${detail.dateKind} ${detail.date || "없음"}` : detail.date || "기준일 없음"; return `<li>${escapeHtml(detail.source)} · ${escapeHtml(shownDate)} · ${escapeHtml(detail.label)}</li>`; }).join("")}</ul></details>`;
  }
  ...
  function mount(container, context) {
    ...
    container.innerHTML = `<section class="company-workspace" data-workspace="company">
      ...
      <section class="primary-chart-card" data-company-chart="edm">
        <div class="chart-card-heading"><div><p class="eyebrow">EDM 괴리 이력</p><h2>시장은 사업보다 얼마나 앞서거나 뒤처졌는가?</h2><p>0 위는 시장 탐욕 선행, 0 아래는 사업 근거 선행입니다.</p></div></div>
    ...
    if (root.EDMCharts) {
      const chart = container.querySelector("#edm-history-chart");
      root.EDMCharts.renderCoordinatedHistory({container: chart, company: payload, frequency: "weekly"});
    }
