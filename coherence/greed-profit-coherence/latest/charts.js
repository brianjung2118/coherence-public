(function (root) {
  ...
  const SERIES = {
    ...
    "expected-ebit": {label: "기대 영업이익", className: "expected-ebit", dash: ""},
  ...
  function renderCoordinatedHistory({container, company, view = "overview", frequency = "weekly", selectedPillars = PILLARS}) {
    clear(container);
    const history = company && company.history;
    const domain = selectedHistory(history, frequency);
    ...
    container.dataset.historyView = view;
    container.dataset.frequency = frequency;
    container.append(freshnessStatus(domain, company));
    const activeKeys = view === "pillars" ? selectedPillars : ["greed", "support"];
    const plotEdm = activeKeys.some((key) => domain.edm.filter((row) => finite(row[`${key}_score`])).length >= 2);
    const plotEarnings = domain.earnings.some((row) => finite(row.expected_operating_profit_krw) || finite(row.actual_operating_profit_krw));
    if (plotEdm || plotEarnings) container.append(legend(view, [...selectedPillars], plotEarnings));
    const allDates = [
    ...
    const scores = plotEdm ? scorePanel(domain.edm, frequency, allDates, view, selectedPillars) : null;
    const earnings = plotEarnings ? earningsPanel(domain.earnings, frequency, allDates) : null;
